...
//  --Generated On Tue Oct 18 17:17:27 IST 2016******************* 
//  **************** Start jsonWriter.js*******************
...
kony.sync.createDownloadTask = function(dbname, tableName, columnName, primaryKeyTable, config, successCallback, errorCallback) {
    ...
    if (!kony.sync.isNullOrUndefined(primaryKeyTable)) {
        ...
        var binaryUrl = kony.sync.getServerURL();
        downloadConfig.URL = binaryUrl;
...
sync.log.loggerEngineInit = function() {
    ...
    function addPersisterErrorCallback(statement) {
        kony.print("Could not add persister, due to the following errors:\n", statement);
    }

    function setConfigErrorCallback(statement) {
    ...
    function setLoggerConfig() {
        ...
        sync.log.setLogLevelForLoggerEngine(kony.sync.log.ALL.value);
    }
...
kony.sync.validatePrimaryKeyField = function(objName, opType, inputAttribute, errorcallback) {
    ...
    for (var p in inputAttribute) {
        if (typeof(inputAttribute[p]) === "object") {
            isError = kony.sync.pkValidationForObjects(objName, inputAttribute, errorcallback);
            break;
        }
        break;
    }
...
kony.sync.pkValidationForObjects = function(objName, inputAttribute, errorcallback) {
        sync.log.trace("Entering kony.sync.pkValidationForObjects");
        var isError = false;
        for (var p in inputAttribute) {
            if (kony.sync.isNullOrUndefined(inputAttribute[p].value)) {
                isError = true;
                var x = new Object();
                x.errorCode = 7003;
                var formatobjcall = objName;
                var y = formatobjcall.lastIndexOf(".");
                formatobjcall = formatobjcall.slice(0, y);
                y = formatobjcall.lastIndexOf(".");
                formatobjcall = formatobjcall.slice(y + 1, formatobjcall.length);
                x.errorMessage = "Primary Key " + p + " not specified in searching an item in " + formatobjcall;
                errorcallback(x);
            }
        }
        return isError;
